...
for i in range(fpf_s.getSize()):
    nuis_s = fpf_s.at(i)
    name   = nuis_s.GetName();
    if not name.rfind("BDTbin") == -1:
       continue;
    if name == "r":
       continue;
    nnuis_to_plot = nnuis_to_plot + 1
    print "nuisance to plot: "+name
    
nuis_p_i=0
# Also make histograms for pull distributions:
hist_fit_b  = ROOT.TH1F("prefit_fit_b"   ,"B-only fit Nuisances;;#theta ",nnuis_to_plot,0,nnuis_to_plot)
hist_fit_s  = ROOT.TH1F("prefit_fit_s"   ,"S+B fit Nuisances   ;;#theta ",nnuis_to_plot,0,nnuis_to_plot)
hist_prefit = ROOT.TH1F("prefit_nuisancs","Prefit Nuisances    ;;#theta ",nnuis_to_plot,0,nnuis_to_plot)
