...
def bestfit():
    ...
    # DL, SL, comb
    mu    = np.array( [ -0.043515, -0.428590, -0.185586 ] )
    upper = np.array( [ 1.4967634, 1.0176909, 0.8035500 ] )
    lower = np.array( [ 1.3923725, 1.0152319, 0.8133152 ] )
    upper_stat = np.array( [ 1.05, 0.51, 0.45 ] )
    lower_stat = np.array( [ 0.96, 0.52, 0.44 ] )
    upper_syst = np.array( [ 1.01, 0.88, 0.66 ] )
    lower_syst = np.array( [ 1.06, 0.87, 0.68 ] )
    channels = np.array( [ 7.5, 4.5, 1.5 ] )
    zero    = np.zeros( nchannels )
    ...
    leg.SetTextAlign( 11 )
    leg.DrawLatex( 2.4, 3*nchannels, "#mu       #color[4]{tot.}    #color[2]{stat.}    syst." )
    ...
    c.Modified()
    c.Update()
    c.SaveAs( "HIG-16-038_bestfit.pdf" )
